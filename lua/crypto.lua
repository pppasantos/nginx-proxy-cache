...
function _M.encrypt(plaintext)
    if not is_enabled() then
        log.log_warn("Encryption is disabled. Skipping.")
        return plaintext
    end
 
    local key, iv_or_err = get_key_iv()
    if not key then
        log.log_err("Encryption skipped: ", iv_or_err)
        return plaintext
    end
    ...
    if not ok then
        log.log_err("Encryption error: ", encrypted)
        return plaintext
    end
 
    log.log_warn("Payload encrypted successfully.")
    return b64.encode(encrypted)
end
 
function _M.decrypt(ciphertext)
    if not is_enabled() then
        log.log_warn("Decryption is disabled. Skipping.")
        return ciphertext
    end
 
    local key, iv_or_err = get_key_iv()
    if not key then
        log.log_err("Decryption skipped: ", iv_or_err)
        return ciphertext
    end
 
    local raw = b64.decode(ciphertext)
    if not raw then
        log.log_err("Failed to base64 decode ciphertext")
        return ciphertext
    end
    ...
    if not ok then
        log.log_err("Decryption error: ", decrypted)
        return ciphertext
    end
 
    log.log_warn("Payload decrypted successfully.")
    return decrypted
end
